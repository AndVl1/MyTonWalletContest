...
__webpack_require__.d(methods_namespaceObject, {
  ...
  fetchDieselState: () => (fetchDieselState),
  fetchNfts: () => (fetchNfts),
  ...
  getStakingState: () => (staking_getStakingState),
  ...
  setupVestingPolling: () => (setupVestingPolling),
  ...
  updateAccountNfts: () => (updateAccountNfts),
...
let errors_ApiCommonError = /*#__PURE__*/function (ApiCommonError) {
...
async function tryMigratingMnemonicEncryption(accountId, mnemonic, password) {
  try {
    ...
    if (!password || !decryptedMnemonic) {
      return {
        error: errors_ApiCommonError.DebugError
      };
    }
...
;// CONCATENATED MODULE: ./src/lib/confusables/util.ts
...
;// CONCATENATED MODULE: ./src/lib/confusables/characters.ts
...
;// CONCATENATED MODULE: ./src/lib/confusables/index.ts
...
class ApiServerError extends ApiBaseError {
  constructor(message, statusCode) {
    super(message, errors_ApiCommonError.ServerError);
    this.statusCode = statusCode;
  }
}
class ApiUnsupportedVersionError extends (/* unused pure expression or super */ null && (ApiBaseError)) {
  constructor(message) {
    super(message, ApiCommonError.UnsupportedVersion);
  }
}
class ApiHardwareBlindSigningNotEnabled extends (/* unused pure expression or super */ null && (ApiBaseError)) {
  constructor() {
...
async function handleFetchErrors(response, ignoreHttpCodes) {
  if (!response.ok && !(ignoreHttpCodes !== null && ignoreHttpCodes !== void 0 && ignoreHttpCodes.includes(response.status))) {
    ...
    throw new ApiServerError((_error = error) !== null && _error !== void 0 ? _error : `HTTP Error ${response.status}`, response.status);
  }
  return response;
}
...
var axios = __webpack_require__(4107);
...
var WalletContractV5R1 = __webpack_require__(4395);
// EXTERNAL MODULE: ./node_modules/axios/lib/core/buildFullPath.js + 2 modules
var buildFullPath = __webpack_require__(6378);
// EXTERNAL MODULE: ./node_modules/axios/lib/core/settle.js
var settle = __webpack_require__(1992);
// EXTERNAL MODULE: ./node_modules/axios/lib/helpers/buildURL.js
var buildURL = __webpack_require__(3343);
// EXTERNAL MODULE: ./node_modules/axios/lib/utils.js
var utils = __webpack_require__(8113);
;// CONCATENATED MODULE: ./src/lib/axios-fetch-adapter/index.js
...
async function fetchAdapter(config) {
  ...
  return new Promise((resolve, reject) => {
    if (data instanceof Error) {
      ...
      Object.prototype.toString.call(config.settle) === '[object Function]' ? config.settle(resolve, reject, data) : (0,settle/* default */.Z)(resolve, reject, data);
    }
  });
}
...
function createRequest(config) {
  ...
  const url = (0,buildURL/* default */.Z)(fullPath, config.params, config.paramsSerializer);
...
function createError(message, config, code, request, response) {
  if (axios/* default.AxiosError */.Z.AxiosError && typeof axios/* default.AxiosError */.Z.AxiosError === 'function') {
    return new axios/* default.AxiosError */.Z.AxiosError(message, axios/* default.AxiosError */.Z.AxiosError[code], config, request, response);
  }
...
;// CONCATENATED MODULE: ./src/api/blockchains/ton/contracts/DnsItem.ts
...
class DnsItem {
  ...
  static createFromConfig(config, code) {
    ...
    return new DnsItem((0,core_dist.contractAddress)(workchain, init), init);
  }
...
axios_retry_default()(axios/* default */.Z, {
  retries: DEFAULT_RETRIES,
...
class TonClient extends client_TonClient/* TonClient */.g {
...
axios/* default.defaults.adapter */.Z.defaults.adapter = fetchAdapter;
...
function buildTokenTransferBody(params) {
  ...
  let builder = new core_dist.Builder().storeUint(JettonOpCode.Transfer, 32).storeUint(queryId || generateQueryId(), 64).storeCoins(tokenAmount).storeAddress(core_dist.Address.parse(toAddress)).storeAddress(core_dist.Address.parse(responseAddress)).storeBit(false).storeCoins(forwardAmount !== null && forwardAmount !== void 0 ? forwardAmount : 0n);
...
function createNestedCell(data, maxCellSize) {
  const builder = new core_dist.Builder();
  const dataSlice = tonCore_Buffer.from(data.slice(0, maxCellSize));
...
function packBytesAsSnakeForEncryptedData(data) {
  ...
  const rootBuilder = new core_dist.Builder();
  rootBuilder.storeBuffer(tonCore_Buffer.from(data.slice(0, Math.min(data.length, ROOT_BUILDER_BYTES))));
  if (data.length > ROOT_BUILDER_BYTES + MAX_CELLS_AMOUNT * TON_MAX_COMMENT_BYTES) {
    throw new Error('Input text is too long');
  }
  rootBuilder.storeRef(createNestedCell(tonCore_Buffer.from(data.slice(ROOT_BUILDER_BYTES)), TON_MAX_COMMENT_BYTES));
...
function buildLiquidStakingWithdrawCustomPayload(waitTillRoundEnd, fillOrKill) {
  return new core_dist.Builder().storeUint(Number(waitTillRoundEnd), 1).storeUint(Number(fillOrKill), 1).asCell();
}
...
async function getDnsItemDomain(network, address) {
  var _Object$entries$find;
  if (typeof address === 'string') address = core_dist.Address.parse(address);
  ...
  const zone = (_Object$entries$find = Object.entries(DNS_ZONES_MAP).find(_ref => {
    let [, collection] = _ref;
      ...
      case DnsOpCode.ChangeRecord:
        {
          ...
          const category = (_ref = (_Object$entries$find = Object.entries(DNS_CATEGORY_HASH_MAP).find(_ref2 => {
            let [, value] = _ref2;
            return hash === value;
          })) === null || _Object$entries$find === void 0 ? void 0 : _Object$entries$find[0]) !== null && _ref !== void 0 ? _ref : 'unknown';
          const toAddress = slice.loadAddress();
          const domain = shouldLoadItems ? await getDnsItemDomain(network, toAddress) : '';
          if (category === 'wallet') {
...
;// CONCATENATED MODULE: ./src/api/blockchains/ton/util/diesel.ts
...
;// CONCATENATED MODULE: ./src/api/blockchains/ton/util/sendExternal.ts
...
async function pickBestWallet(network, publicKey) {
  ...
  const defaultWallet = allWallets.filter(_ref => {
    ...
    return version === DEFAULT_WALLET_VERSION;
  })[0];
...
function resolveWalletVersion(wallet) {
  var _Object$entries$find;
  return (_Object$entries$find = Object.entries(walletClassMap).find(_ref4 => {
...
async function checkTransactionDraft(options) {
  ...
  try {
    ...
    if (!wallet) {
      return {
        ...result,
        error: errors_ApiCommonError.Unexpected
      };
    }
...
async function submitTransferWithDiesel(options) {
  ...
  const [account, keyPair] = await Promise.all([fetchStoredAccount(accountId), fetchKeyPair(accountId, password)]);
  const {
    address: fromAddress
  } = account;
  const {
  ...
  const messages = [omit(await buildTokenTransfer(network, tokenAddress, fromAddress, toAddress, amount, data), ['tokenWallet']), omit(await buildTokenTransfer(network, tokenAddress, fromAddress, DIESEL_ADDRESS, dieselAmount), ['tokenWallet'])];
...
async function stringToPayload(_ref) {
  ...
  if (!data) {
    ...
    encryptedComment = transactions_Buffer.from(payload.slice(4)).toString('base64');
  } else {
...
async function signTransaction(_ref2) {
  ...
  if (payload instanceof Uint8Array) {
    payload = shouldEncrypt ? packBytesAsSnakeForEncryptedData(payload) : packBytesAsSnake(payload, 0);
  }
  const init = stateInit ? {
    code: stateInit.refs[0],
    data: stateInit.refs[1]
  } : undefined;
  const sendMode = isFullBalance ? core_dist.SendMode.CARRY_ALL_REMAINING_BALANCE : core_dist.SendMode.PAY_GAS_SEPARATELY + core_dist.SendMode.IGNORE_ERRORS;
...
async function submitMultiTransfer(accountId, password, messages, expireAt) {
  ...
  try {
    ...
    const {
      msgHash,
      boc
    } = await sendExternal(client, wallet, transaction, withDiesel);
    if (!withDiesel) {
      addPendingTransfer(network, fromAddress, seqno, boc);
    }
...
function toExternalMessage(contract, seqno, body) {
  return (0,core_dist.beginCell)().storeWritable((0,core_dist.storeMessage)((0,core_dist.external)({
    to: contract.address,
...
async function fetchEstimateDiesel(accountId, tokenAddress) {
  ...
  const {
    decimals
  } = findTokenByMinter(tokenAddress);
...
async function submitNftTransfers(options) {
  ...
  const messages = nftAddresses.map((nftAddress, index) => {
    ...
    const isNotcoinBurn = (nft === null || nft === void 0 ? void 0 : nft.collectionAddress) === NOTCOIN_VOUCHERS_ADDRESS && (toAddress === BURN_ADDRESS || NOTCOIN_EXCHANGERS.includes(toAddress));
...
function buildNftTransferPayload(fromAddress, toAddress, payload) {
  let forwardAmount = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : NFT_TRANSFER_TONCOIN_FORWARD_AMOUNT;
  let builder = new core_dist.Builder().storeUint(NftOpCode.TransferOwnership, 32).storeUint(generateQueryId(), 64).storeAddress(core_dist.Address.parse(toAddress)).storeAddress(core_dist.Address.parse(fromAddress)).storeBit(false) // null custom_payload
...
function buildSwapHistoryRange(transactions) {
  ...
  const asset = slug === TONCOIN_SLUG ? TON_SYMBOL : tokens_resolveTokenBySlug(slug).minterAddress;
  return {
    asset,
...
async function swapCexCreateTransaction(accountId, password, request) {
  ...
  const {
    swap
  } = await callBackendPost('/swap/cex/createTransaction', request, {
    authToken
  });
...
async function transactions_submitTransfer(options) {
  ...
  const slug = tokenAddress ? tokens_buildTokenSlug(tokenAddress) : TONCOIN_SLUG;
...
async function staking_getStakingState(accountId) {
  const blockchain = blockchains[resolveBlockchainKey(accountId)];
  const backendState = await getBackendStakingState(accountId);
...
;// CONCATENATED MODULE: ./src/api/methods/vesting.ts
...
async function setupWalletVersionsPolling(accountId) {
  ...
  while (isAlive(localOnUpdate, accountId)) {
    try {
      const allWalletInfos = (await ton.getWalletVersionInfos(network, publicKeyBytes, versions)).map(versionInfo => omit(versionInfo, ['wallet']));
      const filteredInfos = [];
...
async function sendTransaction(request, message) {
  try {
    ...
    const dappNetwork = dappNetworkRaw ? dappNetworkRaw === esm/* CHAIN.MAINNET */.sX.MAINNET ? 'mainnet' : 'testnet' : undefined;
    ...
    if (txPayload.from && toBase64Address(txPayload.from) !== toBase64Address(account.address)) {
      ...
      if (isLedger && (await checkIsHisVestingWallet(network, publicKey, txPayload.from))) {
        ...
        throw new BadRequestError(undefined, errors_ApiTransactionError.WrongAddress);
      }
    }
    if (dappNetwork && network !== dappNetwork) {
      throw new BadRequestError(undefined, errors_ApiTransactionError.WrongNetwork);
    }
    await openExtensionPopup(true);
    onPopupUpdate({
      type: 'dappLoading',
    ...
    if (err instanceof ApiUserRejectsError) {
      ...
      displayError = errors_ApiCommonError.Unexpected;
    }
...
async function checkIsHisVestingWallet(network, ownerPublicKey, address) {
  ...
  const [info, publicKey] = await Promise.all([getContractInfo(network, address), getWalletPublicKey(network, address)]);
